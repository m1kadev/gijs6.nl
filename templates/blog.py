...
def generate_rss_feed():
    ...
    feed.link(
        href="http://www.gijs6.nl/blog/rss.xml", rel="self", type="application/rss+xml"
    )
    feed.link(
        href="http://www.gijs6.nl/blog/atom.xml",
        rel="self",
    ...
    for post in os.listdir(os.path.join(BASE_DIR, "posts")):
        ...
        if post_data.get("date"):
            date = datetime.fromisoformat(str(post_data["date"])).astimezone(
            ...
            try:
                ...
                date = datetime.fromtimestamp(
                    int(lines[-1]), pytz.timezone("Europe/Amsterdam")
                )
                timestamp = int(lines[-1])
            except (ValueError, IndexError):
                # Not published posts
