...
async function refreshAllCollections() {
    const response = await fetch("/justtodo/api/list_all");
    if (!response.ok) {
    ...
    checkBoxes.forEach((cB) => {
        cB.addEventListener("click", function () {
            ...
            fetch("/justtodo/api/set_checked", {
                method: "PUT",
                headers: {
    ...
    deleteButtons.forEach((dB) => {
        dB.addEventListener("click", function () {
            ...
            if (window.confirm(`Do you really want to delete element #${dBlistitemIndex} from collection ${dBcollection}?`)) {
                fetch("/justtodo/api/delete_item", {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    addCollection.addEventListener("click", function () {
        fetch("/justtodo/api/new_collection", {
            method: "POST",
        })
            .then(async (response) => {
                if (!response.ok) {
    ...
    collectionTitles.forEach((cT) => {
        ...
        const executeUpdate = () => {
            ...
            fetch("/justtodo/api/rename_collection", {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    deleteCollectionButtons.forEach((dCB) => {
        dCB.addEventListener("click", function () {
            ...
            if (window.confirm(`Do you really want to delete collection ${dCBcollection} and all its elements?`)) {
                fetch("/justtodo/api/delete_collection", {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                    },
